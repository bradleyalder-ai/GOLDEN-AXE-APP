...
export default function Game21({ onBack, roomPlayers = [] }) {
  ...
  const handleThrow = (zone) => {
    ...
    if (won) {
      // Player finished — remove from active, add to pendingFinishers
      ...
      // Did everyone in this "throw cycle" finish?
      // A throw cycle = everyone who was active at the START of this round has now thrown.
      // We detect end of cycle by checking: is the next active player one who already threw this cycle?
      // Simplest: if newActive is empty OR the next player's index comes before current (wrapped around)
      ...
      if (cycleComplete) {
        // Everyone has had their throw — award places now
        awardPlace(newPending, newActive, podium, g);
      } else {
        // Others still need to throw this cycle — let them go
        setGame(g);
      }
      return;
    }

    // Didn't finish — used up throws?
    if (newThrows.length >= throwsPerTurn) {
      const newActive = game.active; // didn't finish, stays active
      const nextIdx = nextActive(game.currentIdx, game.players, newActive);
      const cycleComplete = !hasActiveAfter(game.currentIdx, game.players, newActive);

      const g = { ...game, scores: newScores, throwCounts: newThrowCounts,
        currentIdx: nextIdx, throwsLeft: throwsPerTurn, turnThrows: [] };

      if (cycleComplete) {
        // End of cycle — award any pending finishers from this cycle
        awardPlace(game.pendingFinishers, newActive, podium, { ...g, pendingFinishers: [] });
      } else {
        setGame(g);
      }
    } else {
  ...
  // Is there any active player AFTER currentIdx (going forward, wrapping)?
