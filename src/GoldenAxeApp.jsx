...
export default function GoldenAxeApp({ groupCode, onLeaveGroup }) {
  ...
  const shopInfo = SHOPS.find(s =>
    ...
    groupCode.startsWith(`${s.walkInPrefix}EVT`)
  ) || null;
  const isShopRoom = !!shopInfo;
  const isEventRoom = false;
...
function LeagueScreen({ players, isAdmin, isManager, activeLeague, setActiveLeague, leagueHistory, setLeagueHistory, dayView, setDayView, onStartMatch, onStartPlayoff, onBack, notify }) {
  const isTablet = useTablet();
