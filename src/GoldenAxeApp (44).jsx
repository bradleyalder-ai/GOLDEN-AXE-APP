...
function MatchScreen({ match, players, settings, onComplete, isAdmin, onBack }) {
  ...
  const handleScore = (zone) => {
    ...
    if (!isSolo) {
      ...
      // ── Executioner's Rule: check if gap is now mathematically unreachable (last round, multi-round matches only) ──
      if (isElimMatch && !executionerRule && maxRounds > 1 && round === maxRounds - 1) {
                      ...
                      const playMatch = () => {
                        ...
                        if (isGrandFinal) {
                          ...
                          if (isFinal && !activeTournament.isPlayoff) {
                            // Championship match — 5 throws, 3 rounds
                            onStartMatch({ id: m.id, p0: m.p0, p1: m.p1, tournamentId: activeTournament.id, matchType: "elimination" },
                              { throwsPerPlayer: 5, roundsPerMatch: 3 });
                          } else {
                            onStartMatch({ id: m.id, p0: m.p0, p1: m.p1, tournamentId: activeTournament.id, matchType: "elimination" }, activeTournament.settings);
                          }
                        }
