...
export default function AroundTheWorld({ onBack, roomPlayers = [] }) {
  ...
  const handleTiebreakerDone = (orderedIds) => {
    ...
    if (newPodium.length >= 3 || newActive.length === 0) {
      ...
      setPodium(newPodium);
      setGame({ ...g, active:newActive, pendingFinishers:[], thrownThisCycle: new Set(),
  ...
  const handleThrow = (thrown) => {
    ...
    const g = { ...game, progress:newProgress, throwCounts:newThrowCounts,
      currentIdx:nextIdx, thrownThisCycle: cycleComplete ? new Set() : newThrown };
  ...
  return (
    <div style={{ minHeight:"100vh", background:DARK, color:"#fff", padding:16, maxWidth:500, margin:"0 auto" }}>
      ...
      {/* Progress bars — tap name to switch thrower */}
          ...
          return (
            <div key={pl.id}
              ...
              <span style={{ color:isCurrent?GOLD:canTap?"#aaa":!isActive?"#444":"#888",
                fontFamily:"monospace", fontSize:11, minWidth:80, flexShrink:0,
                borderBottom: canTap ? "1px dashed #555" : "none" }}>
                {podiumEntry ? MEDALS[podiumEntry.place-1]+" " : isCurrent ? "▶ " : canTap ? "👆 " : ""}{pl.name.slice(0,8)}
              </span>
              <div style={{ flex:1, height:10, background:"#222", borderRadius:6, overflow:"hidden",
                border:`1px solid ${isCurrent?GOLD+"44":canTap?"#55544":"#333"}` }}>
