...
function useLandscape() {
  ...
  useEffect(() => {
    const handler = () => setIsLandscape(window.innerWidth > window.innerHeight && window.innerHeight < 500);
    window.addEventListener("resize", handler);
    return () => window.removeEventListener("resize", handler);
  }, []);
  return isLandscape;
...
function AxeTarget({ onScore, disabled }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center",
      gap: isTablet ? 20 : 12, width: "100%",
      height: isLandscape ? "100%" : "auto" }}>
      <svg
        viewBox="0 0 300 320"
        style={{
          width: "100%",
          flex: isLandscape ? "1 1 0" : "none",
          height: isLandscape ? "0" : "auto",
          maxWidth: isLandscape ? "none" : isTablet ? 480 : 340,
          cursor: disabled ? "not-allowed" : "crosshair",
          filter: disabled ? "grayscale(0.6) opacity(0.7)" : "none",
      ...
      >
        ...
        />
        <text pointerEvents="none" x="150" y="285" textAnchor="middle" fill="#aaa" fontSize="11" fontFamily="monospace">MISS / BOARD (0 pts)</text>
        ...
        <circle
          ...
          onMouseLeave={() => setHover(null)}
        />
        <text pointerEvents="none" x="150" y="160" textAnchor="middle" fill={GOLD} fontSize="13" fontFamily="monospace" fontWeight="bold">●</text>
        ...
        <circle
          ...
          onMouseLeave={() => setHover(null)}
        />
        <text pointerEvents="none" x="55" y="50" textAnchor="middle" fill={GOLD} fontSize="9" fontFamily="monospace" fontWeight="bold">7</text>
        ...
        <circle
          ...
          onMouseLeave={() => setHover(null)}
        />
        <text pointerEvents="none" x="245" y="50" textAnchor="middle" fill={GOLD} fontSize="9" fontFamily="monospace" fontWeight="bold">7</text>

        {/* Ring labels */}
        <text pointerEvents="none" x="150" y="106" textAnchor="middle" fill="rgba(255,255,255,0.5)" fontSize="9" fontFamily="monospace">3 pts</text>
        <text pointerEvents="none" x="150" y="58" textAnchor="middle" fill="rgba(255,255,255,0.5)" fontSize="9" fontFamily="monospace">1 pt</text>
        <text pointerEvents="none" x="150" y="155" textAnchor="middle" fill={GOLD} fontSize="10" fontFamily="monospace" fontWeight="bold">5</text>
      </svg>

      {/* Touch-friendly score buttons — hidden in landscape */}
      {!isLandscape && (
...
// ─── MATCH SCREEN ─────────────────────────────────────────────────────────────
function MatchScreen({ match, players, settings, onComplete, isAdmin, onBack, groupCode }) {
  const isTablet = useTablet();
  const isLandscape = useLandscape();
  ...
  // Try to restore saved match state from Firebase on mount
  ...
  // Restore saved match state on mount
  useEffect(() => {
    if (!groupCode || !match?.id) return;
    import("firebase/database").then(({ getDatabase, ref, get, remove }) => {
      const db = getDatabase();
      get(ref(db, `activeMatch/${groupCode}`)).then(snap => {
        const saved = snap.val();
        if (saved && saved.matchId === match.id) {
          if (saved.scores) setScores(saved.scores);
          if (saved.round !== undefined) setRound(saved.round);
          if (saved.currentPlayer !== undefined) setCurrentPlayer(saved.currentPlayer);
          if (saved.phase) setPhase(saved.phase);
          if (saved.roundWinners) setRoundWinners(saved.roundWinners);
        }
        setRestored(true);
  ...
  // Auto-save match state after every throw
  const saveMatchState = useCallback((newScores, newRound, newPlayer, newPhase, newRoundWinners) => {
    if (!groupCode || !match?.id) return;
    import("firebase/database").then(({ getDatabase, ref, set }) => {
      set(ref(getDatabase(), `activeMatch/${groupCode}`), {
        matchId: match.id,
        scores: newScores,
        round: newRound,
  ...
  // Clear saved match when complete
  const clearSavedMatch = useCallback(() => {
    if (!groupCode) return;
    import("firebase/database").then(({ getDatabase, ref, remove }) => {
      remove(ref(getDatabase(), `activeMatch/${groupCode}`)).catch(() => {});
    });
  }, [groupCode]);
  ...
  const handleScore = (zone) => {
    ...
    setScores(newScores);
    saveMatchState(newScores, round, currentPlayer, phase, roundWinners);
  ...
  const advanceRound = (s0, s1, fromOT = false, fromExecutioner = false) => {
    ...
    // Record winner for this round immediately so tally is always accurate
    setRoundWinners(prev => {
      const next = [...prev];
      next[round] = winnerIdx;
      return next;
    });
  };
  ...
  const confirmRound = () => {
    ...
    if (round + 1 >= maxRounds) {
      // Tally round wins from the roundWinners array — recorded at end of each round
      // This correctly handles OT rounds (scores array shows tied, but roundWinners has the real winner)
      let w0 = 0, w1 = 0;
      for (let ri = 0; ri <= round; ri++) {
        if (roundWinners[ri] === 0) w0++;
        else if (roundWinners[ri] === 1) w1++;
      }
      if (w0 === w1) {
      ...
      // Not last round yet — but check if playoff match already has a winner
      const isPlayoffMatch = !!match.leagueId && !!match.tournamentId;
      if (isPlayoffMatch) {
        let w0 = 0, w1 = 0;
        for (let ri = 0; ri <= round; ri++) {
          if (roundWinners[ri] === 0) w0++;
        ...
        const roundsNeeded = Math.ceil(maxRounds / 2); // e.g. 2 out of 3
        if (w0 >= roundsNeeded || w1 >= roundsNeeded) {
          // Player already won enough rounds — end match now
          finishMatch(null, w0, w1);
          return;
        }
      }
      setPhase("playing");
  ...
  const finishMatch = (otWinner = null, w0override = null, w1override = null) => {
    ...
    clearSavedMatch(); // clear auto-save on completion
  ...
  const scoreBoard = (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      {(isSolo ? [p0] : [p0, p1 ?? p0]).filter((p,i) => !isSolo || i===0).map((p, pi) => {
        const isActive = currentPlayer === pi && phase === "playing";
        const canSwitch = phase === "playing" && isAdmin && !isSolo && !bothDone;
        const alreadyThrown = (scores[round]?.[pi] || []).length >= maxThrows;
        const canTap = canSwitch && !alreadyThrown && !isActive;
        return (
          <div key={pi}
            onClick={() => canTap && setCurrentPlayer(pi)}
            style={{
              background: PANEL,
              border: `2px solid ${isActive ? GOLD : canTap ? "#555" : BORDER}`,
              borderRadius: 10, padding: isTablet ? 16 : isLandscape ? 8 : 12,
              boxShadow: isActive ? `0 0 12px ${GOLD}55` : "none",
              cursor: canTap ? "pointer" : "default",
              transition: "all 0.2s",
            }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: isLandscape ? 4 : 8 }}>
              <span style={{ color: isActive ? GOLD : canTap ? "#aaa" : "#ccc",
                fontFamily: "monospace", fontWeight: "bold", fontSize: isTablet ? 20 : isLandscape ? 13 : 16 }}>
                {isActive ? "🪓 " : canTap ? "👆 " : ""}{p.name}
              </span>
              <div style={{ display: "flex", gap: isLandscape ? 6 : 12 }}>
                {Array.from({ length: maxRounds }).map((_, ri) => (
                  <span key={ri} style={{ color: ri === round ? GOLD : "#666",
                    fontFamily: "monospace", fontSize: isTablet ? 15 : isLandscape ? 11 : 13 }}>
                    R{ri + 1}: <strong style={{ color: "#fff" }}>{totalScore(scores[ri]?.[pi] || [])}</strong>
                  </span>
                ))}
                <span style={{ color: "#aaa", fontFamily: "monospace", fontSize: isTablet ? 15 : isLandscape ? 11 : 13 }}>
            ...
            {canTap && (
              <div style={{ color: "#555", fontFamily: "monospace", fontSize: 10, marginTop: 4, textAlign: "center" }}>
                tap to throw first
              </div>
            )}
          </div>
  ...
  return (
    <div style={{ padding: isLandscape ? "8px 12px" : isTablet ? 24 : 16, maxWidth: isTablet && !isLandscape ? 1000 : "100%", margin: "0 auto" }}>
      {isLandscape && activeTarget ? (
        // ── LANDSCAPE PHONE: left info, right target ──────────────────────
        <div style={{ display: "flex", gap: 10, height: "100vh", boxSizing: "border-box", padding: "6px 8px" }}>
          {/* Left panel — round tabs + score cards + back button at bottom */}
          <div style={{ width: "44%", display: "flex", flexDirection: "column", gap: 6 }}>
            {/* Round tabs + match title compact row */}
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              {roundTabs}
              <span style={{ color: GOLD, fontFamily: "monospace", fontWeight: "bold", fontSize: 11, flex: 1, textAlign: "right" }}>
                {isSolo ? p0.name : `${p0.name} vs ${p1?.name}`}
              </span>
            </div>
            {soloBlock}
            {/* Score cards - take up available space */}
            <div style={{ flex: 1 }}>
              {scoreBoard}
            </div>
            {messageBlock}
            ...
            <button onClick={onBack} style={{ ...btnStyle("#333"), width: "100%", padding: "8px", fontSize: 12 }}>
              ← Back
            </button>
          </div>
          {/* Right panel — target fills right side */}
          <div style={{
            flex: 1,
            display: "flex",
            alignItems: "stretch",
...
// ─── MAIN APP ─────────────────────────────────────────────────────────────────
// ─── HUB PLAYER PANEL ────────────────────────────────────────────────────────
// Simple add/remove player list shown on the event/walk-in hub
function HubPlayerPanel({ players, setPlayers, isScorer }) {
  const [newName, setNewName] = useState("");
  const GOLD = "#f0c040", DARK = "#0d0d0d";

  const addPlayer = () => {
    if (!newName.trim()) return;
    setPlayers(prev => [...prev, { id: Date.now().toString(), name: newName.trim(), createdAt: Date.now() }]);
    setNewName("");
  };
  ...
  return (
    <div style={{ background: "#111", border: `1px solid #2a2a2a`,
      ...
      <div style={{ display: "flex", justifyContent: "space-between",
        ...
        <div style={{ color: GOLD, fontFamily: "monospace", fontWeight: "bold", fontSize: 13 }}>
          🪓 PLAYERS ({players.length})
        </div>
        {players.length > 0 && isScorer && (
      ...
      {isScorer && (
        <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
          <input value={newName}
            ...
            style={{ flex: 1, background: "#1a1a1a", border: `1px solid ${GOLD}33`,
              borderRadius: 8, padding: "10px 12px", color: "#fff",
              fontFamily: "monospace", fontSize: 14, outline: "none" }} />
          <button onClick={addPlayer}
            style={{ background: newName.trim() ? GOLD : "#222",
              color: newName.trim() ? DARK : "#555", border: "none",
              borderRadius: 8, padding: "10px 18px", fontFamily: "monospace",
              fontWeight: "bold", fontSize: 20, cursor: newName.trim() ? "pointer" : "default" }}>
            +
          </button>
        </div>
      )}
      ...
      {players.length === 0 && (
        <div style={{ color: "#444", fontFamily: "monospace", fontSize: 12,
          ...
          {isScorer ? "Type a name above and tap + to add players" : "No players yet"}
        </div>
      )}
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
        {players.map((p, i) => (
          <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 5,
            ...
            padding: "5px 10px" }}>
            <span style={{ color: "#555", fontFamily: "monospace", fontSize: 10 }}>#{i+1}</span>
            <span style={{ color: "#ccc", fontFamily: "monospace", fontSize: 13 }}>{p.name}</span>
            {isScorer && (
              <span onClick={() => setPlayers(prev => prev.filter(x => x.id !== p.id))}
                style={{ color: "#555", cursor: "pointer", fontSize: 15,
                  lineHeight: 1, marginLeft: 3, padding: "0 2px" }}>×</span>
            )}
          </div>
        ))}
      </div>
...
export default function GoldenAxeApp({ groupCode, onLeaveGroup }) {
  ...
  // Intercept Android hardware back button — navigate within app instead of closing
  useEffect(() => {
    // Push a history entry so the back button fires popstate instead of closing
    window.history.pushState({ screen }, "", window.location.href);
    const handlePopState = () => {
      // Re-push so back button always works
      window.history.pushState({ screen }, "", window.location.href);
      // Navigate back within the app
      if (screen === "match") { setScreen("home"); setCurrentMatch(null); return; }
      if (screen === "tournament") { setScreen("hub"); return; }
      if (screen === "league") { setScreen("hub"); return; }
      if (screen === "home") { setScreen("hub"); return; }
      if (screen === "stats") { setScreen("home"); return; }
      if (screen === "settings") { setScreen("hub"); return; }
      if (screen === "leaderboard") { setScreen("hub"); return; }
      if (screen === "share") { setScreen("home"); return; }
      // Already on hub — do nothing (don't close the app)
    };
  ...
  // Firebase sync
  const isSyncingRef = useRef(false);   // true while applying remote data — blocks local pushes
  const hasLoadedRef = useRef(false);    // true once we've received first Firebase snapshot
  const syncTimeouts = useRef({});       // per-field debounce timers

  // Push a single field to Firebase — debounced per field, blocked during remote sync
  const pushField = useCallback((field, value) => {
    if (isSyncingRef.current || !hasLoadedRef.current) return;
    if (syncTimeouts.current[field]) clearTimeout(syncTimeouts.current[field]);
    syncTimeouts.current[field] = setTimeout(() => {
      writeField(groupCode, field, value)
        .catch(err => console.error(`Firebase write error (${field}):`, err));
    }, 600);
  }, [groupCode]);

  // Subscribe to Firebase on mount — receive remote changes and apply them
  useEffect(() => {
    if (!groupCode) return;

    const unsub = subscribeToGroup(groupCode, (data) => {
      if (!data) {
        // Group doesn't exist yet — we are the first device, safe to push our local state
        hasLoadedRef.current = true;
        writeGroupState(groupCode, { players, completedMatches, activeTournament, activeLeague, leagueHistory })
          .catch(err => console.error("Firebase init error:", err));
        setSynced(true);
        return;
      }

      // Remote data received — apply it, blocking any outgoing pushes while we do
      ...
      // Use "in" check so null values (cleared tournament/league) are also applied
      ...
      if (data.roomPins) { setRoomPins(data.roomPins); }
      if (data.roomLogo !== undefined) { setRoomLogo(data.roomLogo || ""); }
      if (data.roomName !== undefined) { setRoomName(data.roomName || ""); }

      // Unblock outgoing pushes after state has settled
      setTimeout(() => { isSyncingRef.current = false; }, 300);
    });

    return () => {
      unsub();
      // Clear any pending debounced writes on unmount
      Object.values(syncTimeouts.current).forEach(clearTimeout);
    };
  }, [groupCode]);

  // Push individual fields when they change — each field is independent so
  // simultaneous scorers on different devices don't overwrite each other's data
  ...
  useEffect(() => {
    ...
    if (champName) {
      submitLeaderboardEntry(shopInfo?.settingsCode || groupCode, "tournamentWins", champName, 1,
  ...
  useEffect(() => {
    ...
    if (champName) {
      submitLeaderboardEntry(shopInfo?.settingsCode || groupCode, "leagueWins", champName, 1,
  ...
  // Role-based access: guest | scorer | manager | master
  ...
  // Look up shop info from directory — must be before any useEffect that uses it
  const shopInfo = SHOPS.find(s =>
    s.settingsCode === groupCode ||
    groupCode.startsWith(s.walkInPrefix) ||
    groupCode === `GA_${s.walkInPrefix}_01`  // legacy room code format
  ) || null;

  // Room type — drives which hub UI shows
  const isLeagueRoom = shopInfo
    ? groupCode.startsWith(`${shopInfo.walkInPrefix}LG`)
    : false;
  const isWalkInRoom = shopInfo && !isLeagueRoom
    ? /^[A-Z]{2,4}\d{3,}$/.test(groupCode)
    : false;
  const isEventRoom  = shopInfo && !isLeagueRoom && !isWalkInRoom;
  ...
  const handleMatchComplete = (result) => {
    ...
    const lbShopCode = shopInfo?.settingsCode || null;
  ...
  if (screen === "leaderboard") return (
    <Leaderboard
      shopRoomCode={shopInfo?.settingsCode || groupCode}
  ...
  if (screen === "hub") return (
    <div style={{ minHeight: "100vh", background: "#0a0a0a", color: "#fff", padding: isTablet ? 40 : 20, maxWidth: 600, margin: "0 auto" }}>
      {showPinModal && <PinModal roomPins={roomPins} currentRole={role}
        onUnlock={r => { setRole(r); setShowPinModal(false); notify(`${ROLE_LABELS[r].label} unlocked!`); }}
        onClose={() => setShowPinModal(false)} />}

      {/* Room Settings Overlay */}
      {showHubSettings && canManage(role) && (
        <div style={{ position: "fixed", inset: 0, zIndex: 400, background: "rgba(0,0,0,0.95)",
          overflowY: "auto", padding: 24 }}>
          <div style={{ maxWidth: 420, margin: "0 auto" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h2 style={{ color: GOLD, fontFamily: "serif", margin: 0 }}>⚙️ Room Settings</h2>
              <button onClick={() => setShowHubSettings(false)} style={{ background: "#333", color: "#fff",
                border: "none", borderRadius: 8, padding: "8px 16px", fontFamily: "monospace",
                fontWeight: "bold", cursor: "pointer" }}>✕ Close</button>
            </div>

            {/* Shop Identity */}
            <div style={{ background: "#111", border: `1px solid #333`, borderRadius: 12, padding: 20, marginBottom: 14 }}>
              <div style={{ color: GOLD, fontFamily: "monospace", fontWeight: "bold", fontSize: 13, marginBottom: 14 }}>
                🏪 SHOP IDENTITY
              </div>
              <div style={{ marginBottom: 10 }}>
                <label style={{ color: "#888", fontFamily: "monospace", fontSize: 11, display: "block", marginBottom: 4 }}>Shop / Venue Name</label>
                <input type="text" placeholder="e.g. Blade & Axe Co." value={roomName}
              ...
              <div style={{ marginBottom: 12 }}>
                <label style={{ color: "#888", fontFamily: "monospace", fontSize: 11, display: "block", marginBottom: 4 }}>Logo Image URL</label>
                <input type="text" placeholder="https://yoursite.com/logo.png" value={roomLogo}
                  onChange={e => setRoomLogo(e.target.value)}
                  style={{ width: "100%", background: "#1a1a1a", border: "1px solid #444", borderRadius: 8,
                    padding: "10px 12px", color: "#fff", fontFamily: "monospace", fontSize: 12,
                    boxSizing: "border-box" }} />
                <div style={{ color: "#555", fontFamily: "monospace", fontSize: 10, marginTop: 4 }}>
                  Tip: Upload to imgur.com, copy the image link
                </div>
              </div>
              {roomLogo ? (
                <div style={{ textAlign: "center", marginBottom: 12 }}>
                  <img src={roomLogo} alt="Logo preview"
                    onError={e => { e.target.style.display = "none"; }}
                    style={{ maxWidth: 180, maxHeight: 80, objectFit: "contain",
                      borderRadius: 8, border: "1px solid #333" }} />
                  <div style={{ color: "#555", fontFamily: "monospace", fontSize: 10, marginTop: 4 }}>Preview</div>
                </div>
              ) : null}
            </div>
            ...
            <div style={{ background: "#111", border: `1px solid #333`, borderRadius: 12, padding: 20, marginBottom: 14 }}>
              ...
              <div style={{ color: "#666", fontFamily: "monospace", fontSize: 11, marginBottom: 12, lineHeight: 1.6 }}>
                Share Scorer PIN with staff. Keep Manager PIN private.
              </div>
              <div style={{ marginBottom: 10 }}>
                <label style={{ color: "#4a90d9", fontFamily: "monospace", fontSize: 11, display: "block", marginBottom: 4 }}>🪓 Scorer PIN</label>
                <input type="text" placeholder="e.g. THROW" value={roomPins.scorerPin || ""}
                  onChange={e => setRoomPins(prev => ({ ...prev, scorerPin: e.target.value }))}
                  style={{ width: "100%", background: "#1a1a1a", border: "1px solid #444", borderRadius: 8,
                    padding: "10px 12px", color: "#fff", fontFamily: "monospace", fontSize: 16,
                    letterSpacing: 3, boxSizing: "border-box" }} />
              </div>
              <div>
                <label style={{ color: GOLD, fontFamily: "monospace", fontSize: 11, display: "block", marginBottom: 4 }}>⚙️ Manager PIN</label>
                <input type="text" placeholder="e.g. AXE2024" value={roomPins.managerPin || ""}
                  onChange={e => setRoomPins(prev => ({ ...prev, managerPin: e.target.value }))}
                  style={{ width: "100%", background: "#1a1a1a", border: "1px solid #444", borderRadius: 8,
                    padding: "10px 12px", color: "#fff", fontFamily: "monospace", fontSize: 16,
                    letterSpacing: 3, boxSizing: "border-box" }} />
              </div>
            ...
            <button onClick={() => {
              Promise.all([
                writeField(groupCode, "roomPins", roomPins),
                writeField(groupCode, "roomLogo", roomLogo),
                writeField(groupCode, "roomName", roomName),
              ])
                .then(() => { notify("✅ Room settings saved!"); setShowHubSettings(false); })
                .catch(() => notify("❌ Error saving — check connection"));
            }} style={{ width: "100%", background: GOLD, color: DARK, border: "none", borderRadius: 12,
              padding: "16px", fontFamily: "monospace", fontWeight: "bold", fontSize: 16, cursor: "pointer" }}>
              💾 Save Room Settings
            </button>
          </div>
        </div>
      ...
      <div style={{ textAlign: "center", marginBottom: 24 }}>
        {(roomLogo || shopInfo?.logo)
          ? <img src={roomLogo || shopInfo.logo} alt="Logo"
              onError={e => e.target.style.display="none"}
              style={{ maxWidth: isTablet ? 200 : 150, maxHeight: 90, objectFit: "contain",
                marginBottom: 8, borderRadius: 10 }} />
          : <div style={{ fontSize: isTablet ? 64 : 48, marginBottom: 8 }}>
              {isLeagueRoom ? "🏅" : "🪓"}
            </div>
        }
        <h1 style={{ color: GOLD, fontFamily: "Georgia, serif",
          fontSize: isTablet ? 32 : 24, margin: "0 0 4px", letterSpacing: 2,
          textTransform: "uppercase" }}>
          {roomName || shopInfo?.name || "Golden Axe"}
        </h1>
        <div style={{ color: "#555", fontFamily: "monospace", fontSize: 11, letterSpacing: 2,
          marginBottom: 10 }}>
          {isLeagueRoom ? "LEAGUE ROOM" : isWalkInRoom ? "WALK-IN SESSION" : "AXE THROWING EVENT"}
        </div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center",
          gap: 8, flexWrap: "wrap" }}>
          <div style={{ background: "#111", border: `1px solid ${isLeagueRoom ? "#1d6a96" : GOLD}44`,
            borderRadius: 8, padding: "4px 12px", fontFamily: "monospace", fontSize: 11,
            color: isLeagueRoom ? "#4a90d9" : GOLD }}>
            <span style={{ color: synced ? "#4f4" : "#f84", marginRight: 4, fontSize: 9 }}>●</span>
            {groupCode}
          </div>
          <button onClick={() => setShowPinModal(true)} style={{
            background: "#111", border: `1px solid ${ROLE_LABELS[role]?.color || "#333"}`,
            borderRadius: 8, padding: "4px 10px", fontFamily: "monospace", fontSize: 11,
            color: ROLE_LABELS[role]?.color || "#888", cursor: "pointer" }}>
            {ROLE_LABELS[role]?.label} 🔐
          </button>
          <button onClick={onLeaveGroup} style={{ background: "transparent",
            border: "1px solid #333", borderRadius: 8, padding: "4px 10px",
            color: "#555", fontFamily: "monospace", fontSize: 11, cursor: "pointer" }}>
            ← Locations
          </button>
        </div>
      </div>
      ...
      {/* ── LEAGUE ROOM HUB ─────────────────────────────────── */}
      {isLeagueRoom && (<>
        <div style={{ background: "#050a12", border: "2px solid #1d6a96", borderRadius: 12,
          padding: "12px 16px", marginBottom: 16, textAlign: "center" }}>
          <div style={{ color: "#4a90d9", fontFamily: "monospace", fontSize: 12,
            fontWeight: "bold" }}>🏅 LEAGUE ROOM · {groupCode}</div>
          <div style={{ color: "#2a4a6a", fontFamily: "monospace", fontSize: 11, marginTop: 4 }}>
            League schedule, standings and playoff live here
          </div>
        </div>

        <div style={{ color: "#4a90d9", fontFamily: "monospace", fontSize: 11,
          fontWeight: "bold", letterSpacing: 2, marginBottom: 10 }}>📅 LEAGUE</div>
        <div onClick={() => setScreen("league")} style={{
          background: "#0a0a1a", border: "2px solid #1d6a96", borderRadius: 14,
          padding: isTablet ? "20px 24px" : "16px 18px", marginBottom: 16,
          cursor: "pointer", display: "flex", alignItems: "center", gap: 16 }}>
          <span style={{ fontSize: 40, flexShrink: 0 }}>📅</span>
          <div>
            <div style={{ color: "#4a90d9", fontFamily: "monospace", fontWeight: "bold",
              fontSize: isTablet ? 18 : 15, marginBottom: 3 }}>League Schedule</div>
            <div style={{ color: "#555", fontFamily: "monospace", fontSize: 11 }}>
              Matches · Standings · Playoff · Season history
            </div>
          </div>
        </div>

        <div style={{ color: "#888", fontFamily: "monospace", fontSize: 11,
          fontWeight: "bold", letterSpacing: 2, marginBottom: 10 }}>🎮 MINI GAMES</div>
        {[
          { key:"game21",      icon:"🎯", title:"21",               border:"#4f4",    bg:"#001a0a", desc:"Easy · Medium · Hard" },
          { key:"aroundworld", icon:"🌍", title:"Around the World", border:"#4fc3f7", bg:"#00101a", desc:"Hit every zone in order" },
          { key:"zombie",      icon:"🧟", title:"Zombie Hunter",    border:"#e63946", bg:"#1a0000", desc:"Champion vs the Horde" },
        ].map(g => (
          <div key={g.key} onClick={() => setScreen(g.key)} style={{
            background: g.bg, border: `2px solid ${g.border}`, borderRadius: 14,
            padding: "14px 18px", marginBottom: 10, cursor: "pointer",
            display: "flex", alignItems: "center", gap: 14 }}>
            <span style={{ fontSize: 36, flexShrink: 0 }}>{g.icon}</span>
            <div>
              <div style={{ color: g.border, fontFamily: "monospace", fontWeight: "bold",
                fontSize: 15, marginBottom: 2 }}>{g.title}</div>
        ...
        <div onClick={() => setScreen("leaderboard")} style={{
          ...
          <div>
            <div style={{ color: GOLD, fontFamily: "monospace", fontWeight: "bold", fontSize: 15, marginBottom: 2 }}>Leaderboard</div>
            <div style={{ color: "#555", fontFamily: "monospace", fontSize: 11 }}>Top 100 · Match · Tournament · League</div>
          </div>
        </div>
      </>)}

      {/* ── EVENT / WALK-IN ROOM HUB ────────────────────────── */}
      {!isLeagueRoom && (<>
        {isWalkInRoom && (
          <div style={{ background: "#0a1a0a", border: "2px solid #39ff14", borderRadius: 12,
            padding: "12px 16px", marginBottom: 16, textAlign: "center" }}>
            <div style={{ color: "#39ff14", fontFamily: "monospace", fontSize: 12, fontWeight: "bold" }}>
              🚶 WALK-IN SESSION · {groupCode}
            </div>
            <div style={{ color: "#446644", fontFamily: "monospace", fontSize: 11, marginTop: 4 }}>
              Add players below and run your tournament or mini games
            </div>
          </div>
        )}

        {/* ── PLAYERS (event/walk-in rooms only) ─────────────── */}
        {!isLeagueRoom && (
          <HubPlayerPanel
            players={players}
        ...
        <div style={{ color: GOLD, fontFamily: "monospace", fontSize: 11,
          fontWeight: "bold", letterSpacing: 2, marginBottom: 10 }}>⚔️ COMPETITIVE</div>
        <div onClick={() => setScreen("home")} style={{
          background: "#1a1500", border: `2px solid ${GOLD}`, borderRadius: 14,
          padding: isTablet ? "20px 24px" : "16px 18px", marginBottom: 10,
          cursor: "pointer", display: "flex", alignItems: "center", gap: 16 }}>
          <span style={{ fontSize: 40, flexShrink: 0 }}>🏆</span>
          <div>
            <div style={{ color: GOLD, fontFamily: "monospace", fontWeight: "bold",
              fontSize: isTablet ? 18 : 15, marginBottom: 3 }}>Golden Axe Tournament</div>
            <div style={{ color: "#666", fontFamily: "monospace", fontSize: 11 }}>
              Single or Double Elimination · Seeding rounds · Champion banner
            </div>
          </div>
        </div>

        <div style={{ color: "#888", fontFamily: "monospace", fontSize: 11,
          fontWeight: "bold", letterSpacing: 2, margin: "16px 0 10px" }}>🎮 MINI GAMES</div>
        {[
          { key:"game21",      icon:"🎯", title:"21",               border:"#4f4",    bg:"#001a0a", desc:"Easy · Medium · Hard modes · Any number of players" },
          { key:"aroundworld", icon:"🌍", title:"Around the World", border:"#4fc3f7", bg:"#00101a", desc:"Hit every zone in order · 3 modes · Side targeting" },
          { key:"zombie",      icon:"🧟", title:"Zombie Hunter",    border:"#e63946", bg:"#1a0000", desc:"Champion vs the Horde · Nipple = headshot · Horde grows!" },
        ].map(g => (
          <div key={g.key} onClick={() => setScreen(g.key)} style={{
            background: g.bg, border: `2px solid ${g.border}`, borderRadius: 14,
            padding: isTablet ? "18px 24px" : "14px 18px", marginBottom: 10,
            cursor: "pointer", display: "flex", alignItems: "center", gap: 16 }}>
            <span style={{ fontSize: 40, flexShrink: 0 }}>{g.icon}</span>
            <div>
              <div style={{ color: g.border, fontFamily: "monospace", fontWeight: "bold",
                fontSize: isTablet ? 18 : 15, marginBottom: 3 }}>{g.title}</div>
              <div style={{ color: "#555", fontFamily: "monospace", fontSize: 11 }}>{g.desc}</div>
            </div>
          </div>
        ))}
      ...
      <div style={{ textAlign: "center", marginTop: 20, color: "#333",
        ...
        {canManage(role) && (
          <button onClick={() => setShowHubSettings(true)} style={{
            display: "block", margin: "10px auto 0", background: "#111",
            border: "1px solid #333", borderRadius: 8, padding: "8px 20px",
            color: "#888", fontFamily: "monospace", fontSize: 12, cursor: "pointer" }}>
            ⚙️ Room Settings
          </button>
        )}
      </div>
  ...
  if (screen === "home") return (
    ...
    <div style={{ minHeight: "100vh", background: t.bg, color: t.text, fontFamily: "monospace" }}>
      ...
      <div style={{ background: t.headerBg, borderBottom: `2px solid ${GOLD}`, padding: "20px 24px", textAlign: "center" }}>
        ...
        {roomLogo
          ? <img src={roomLogo} alt="Shop Logo"
              onError={e => { e.target.style.display = "none"; }}
              style={{ maxWidth: isTablet ? 180 : 130, maxHeight: 70, objectFit: "contain",
        ...
        <div style={{ marginTop: 10, display: "flex", justifyContent: "center", gap: 8, flexWrap: "wrap" }}>
          <button onClick={() => setShowPinModal(true)} style={{
            background: "#1a1a1a", border: `1px solid ${ROLE_LABELS[role]?.color || "#555"}`,
            borderRadius: 6, padding: "2px 10px", fontSize: 11,
            color: ROLE_LABELS[role]?.color || "#aaa", cursor: "pointer" }}>
            {ROLE_LABELS[role]?.label} 🔐
          </button>
      ...
      {/* Pin Modal */}
      {(pinModal || showPinModal) && (
        <PinModal roomPins={roomPins} currentRole={role}
          onUnlock={r => { setRole(r); setPinModal(false); setShowPinModal(false); notify(`${ROLE_LABELS[r].label} unlocked!`); }}
          onClose={() => { setPinModal(false); setShowPinModal(false); }} />
      )}
  ...
  if (screen === "share") return (
    <div style={{ minHeight: "100vh", background: DARK, color: "#fff", padding: isTablet ? 32 : 20, maxWidth: isTablet ? 800 : 500, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={cardStyle}>
          <h3 style={{ color: GOLD, margin: "0 0 10px" }}>🔐 Room PINs</h3>
          {!canManage(role) && <div style={{ color: "#888", fontFamily: "monospace", fontSize: 12 }}>Manager or Master access required to set PINs.</div>}
          {canManage(role) && (<>
            <div style={{ color: "#888", fontFamily: "monospace", fontSize: 11, marginBottom: 12, lineHeight: 1.6 }}>
              Set PINs for this room. Share Scorer PIN with staff, Manager PIN with coaches.
            </div>
            <div style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "center" }}>
              <span style={{ color: GOLD, fontFamily: "monospace", fontSize: 12, minWidth: 80 }}>🏪 Name</span>
              <input type="text" placeholder="Shop / Venue name" value={roomName}
                onChange={e => setRoomName(e.target.value)}
                style={{ ...inputStyle, flex: 1 }} />
            </div>
            <div style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "center" }}>
              <span style={{ color: GOLD, fontFamily: "monospace", fontSize: 12, minWidth: 80 }}>🖼️ Logo</span>
              <input type="text" placeholder="Paste image URL (https://...)" value={roomLogo}
                onChange={e => setRoomLogo(e.target.value)}
                style={{ ...inputStyle, flex: 1, fontSize: 11 }} />
            </div>
            {roomLogo ? (
              <div style={{ marginBottom: 12, textAlign: "center" }}>
                <img src={roomLogo} alt="Preview"
                  onError={e => { e.target.style.display = "none"; }}
                  style={{ maxWidth: 160, maxHeight: 70, objectFit: "contain", borderRadius: 8,
                    border: "1px solid #333" }} />
                <div style={{ color: "#555", fontFamily: "monospace", fontSize: 10, marginTop: 4 }}>Logo preview</div>
              </div>
            ) : null}
            <div style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "center" }}>
              <span style={{ color: "#4a90d9", fontFamily: "monospace", fontSize: 12, minWidth: 80 }}>🪓 Scorer</span>
              <input type="password" placeholder="Scorer PIN" value={roomPins.scorerPin || ""}
                onChange={e => setRoomPins(prev => ({ ...prev, scorerPin: e.target.value }))}
                style={{ ...inputStyle, flex: 1 }} />
            </div>
            <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center" }}>
              <span style={{ color: GOLD, fontFamily: "monospace", fontSize: 12, minWidth: 80 }}>⚙️ Manager</span>
              <input type="password" placeholder="Manager PIN" value={roomPins.managerPin || ""}
                onChange={e => setRoomPins(prev => ({ ...prev, managerPin: e.target.value }))}
                style={{ ...inputStyle, flex: 1 }} />
            </div>
            <button onClick={() => {
              Promise.all([
                ...
                .then(() => notify("Room settings saved!"))
                .catch(() => notify("Error saving settings"));
            }} style={{ ...btnStyle(GOLD, DARK), width: "auto", padding: "8px 20px" }}>
              💾 Save Room Settings
            </button>
          </>)}
        </div>
  ...
  if (screen === "tournament" || activeTournament?.isPlayoff) return (
    <TournamentScreen
      players={players} isAdmin={isScorer} isManager={isAdmin}
      activeTournament={activeTournament}
  ...
  if (screen === "league") return (
    <LeagueScreen
      players={players} setPlayers={setPlayers} isAdmin={isScorer} isManager={isAdmin} role={role}
...
function progressDoubleElim(tournament, matchId, winnerId, loserId) {
  ...
  t.wBracket = t.wBracket || [];
  t.lBracket = t.lBracket || [];

  // ── Find which bracket/round/slot this match is in ──────────────────────
  let src = null;
  for (let ri = 0; ri < t.wBracket.length && !src; ri++)
    for (let mi = 0; mi < t.wBracket[ri].length && !src; mi++)
      if (t.wBracket[ri][mi].id === matchId) src = { b:"w", ri, mi };
  for (let ri = 0; ri < t.lBracket.length && !src; ri++)
    for (let mi = 0; mi < t.lBracket[ri].length && !src; mi++)
      if (t.lBracket[ri][mi].id === matchId) src = { b:"l", ri, mi };
  if (!src) return t;

  // Mark match complete
  const arr = src.b === "w" ? t.wBracket : t.lBracket;
  arr[src.ri][src.mi] = { ...arr[src.ri][src.mi], completed: true, winner: winnerId };

  if (src.b === "w") {
    // ── WINNERS BRACKET match completed ─────────────────────────────────
    // 1. Loser drops to losers bracket
    //    WB round 0 losers fill LB round 0 slots (paired immediately when 2 arrive)
    //    WB round 1 losers fill LB round 2 slots (after LB round 1 plays)
    //    WB round N losers fill LB round N*2 slots
    const lbDropInRound = src.ri * 2;
    if (!t.lBracket[lbDropInRound]) t.lBracket[lbDropInRound] = [];
    t.lBracket[lbDropInRound].push({
      id: `lb_r${lbDropInRound}_s${t.lBracket[lbDropInRound].length}_${Date.now()}`,
      p0: loserId, p1: null, completed: false, byeMatch: false, fromWB: true,
    });

    // 2. Try to pair LB drop-ins in this round (need exactly 2 to make a real match)
    const lbRound = t.lBracket[lbDropInRound];
    const waiting = lbRound.filter(m => m.p1 === null && !m.byeMatch && !m.completed);
    if (waiting.length >= 2) {
      // Pair first two waiting players
      const [a, b] = waiting;
      const aIdx = lbRound.indexOf(a);
      lbRound[aIdx] = { ...a, p1: b.p0 };
      lbRound.splice(lbRound.indexOf(b), 1);
    }
    // If only 1 waiting — they stay waiting for next WB loser. NO byes yet.

    // 3. Check if WB round is fully done → generate next WB round
    if (t.wBracket[src.ri].every(m => m.completed)) {
      const wbWinners = t.wBracket[src.ri].map(m => m.winner).filter(Boolean);
      if (wbWinners.length === 1) {
        // Only one WB winner → WB is over, this is the finalist
        t.wbFinalist = wbWinners[0];
        // If LB is also done, set up grand final
        const allLBDone = t.lBracket.every(r => r.every(m => m.completed));
        if (allLBDone && t.lBracket.length > 0) {
          const lastLBRound = t.lBracket[t.lBracket.length - 1];
          const lbWinner = lastLBRound.find(m => m.completed)?.winner;
          if (lbWinner) {
            t.grandFinal = {
              id: "grand_final", p0: t.wbFinalist, p1: lbWinner,
              completed: false, byeMatch: false,
            };
          }
        }
        ...
        for (let i = 0; i < wbWinners.length; i += 2) {
          if (wbWinners[i+1]) {
            nextWB.push({ id: `w_r${src.ri+1}_s${i/2}_${Date.now()}`,
              p0: wbWinners[i], p1: wbWinners[i+1], completed: false, byeMatch: false });
          }
        }
        if (nextWB.length) t.wBracket.push(nextWB);
      }
    }

  } else {
    // ── LOSERS BRACKET match completed ──────────────────────────────────
    // 1. Loser is eliminated — do nothing with them
    // 2. Check if this LB round is fully done (all real matches, not just bye placeholders)
    const lbRound = t.lBracket[src.ri];
    const realMatches = lbRound.filter(m => !m.byeMatch);
    const allRealDone = realMatches.every(m => m.completed);

    if (allRealDone && realMatches.length > 0) {
      const lbWinners = realMatches.map(m => m.winner).filter(Boolean);

      // Check if WB finalist exists and LB is at final stage
      ...
      } else if (lbWinners.length > 0) {
        // Generate next LB round — this is a "LB vs LB" consolidation round
        // Then after that, the next WB drop-in round will follow
        const nextLBRound = src.ri + 1;
        if (!t.lBracket[nextLBRound]) t.lBracket[nextLBRound] = [];

        // Place LB winners into next round slots
        for (const w of lbWinners) {
          t.lBracket[nextLBRound].push({
            id: `lb_r${nextLBRound}_s${t.lBracket[nextLBRound].length}_${Date.now()}`,
            p0: w, p1: null, completed: false, byeMatch: false,
          });
        }

        // Try to pair them
        const nextRound = t.lBracket[nextLBRound];
        const waiting = nextRound.filter(m => m.p1 === null && !m.byeMatch);
        if (waiting.length >= 2) {
          const [a, b] = waiting;
          const aIdx = nextRound.indexOf(a);
          nextRound[aIdx] = { ...a, p1: b.p0 };
          nextRound.splice(nextRound.indexOf(b), 1);
        }
        // If only 1, they wait for next WB dropin (which will arrive in lbDropInRound = nextLBRound)
      }
    }
  }
...
function StatsScreen({ players, completedMatches, leagueHistory, activeTournament, activeLeague, onBack, isTablet, t }) {
  ...
  // Name lookup — IDs in league matches are the player names themselves
  // Fall back to shared players list for legacy rooms, then return id as last resort
  const getPlayerName = (id) => {
    if (!id) return "?";
    // Check active league memberNames (legacy format)
    if (activeLeague?.memberNames?.[id]) return activeLeague.memberNames[id];
    // Check shared players
    const fromPlayers = players.find(p => p.id === id)?.name;
    if (fromPlayers) return fromPlayers;
    // New format: id IS the name (for league members added after this fix)
    // If id looks like a name (not a timestamp), return it directly
    if (!/^\d{13}/.test(id) && !/^test_/.test(id)) return id;
    return "?";
  };
...
function TournamentScreen({ players, isAdmin, isManager, activeTournament, setActiveTournament, completedMatches, onStartMatch, onBack, notify }) {
  const isTablet = useTablet();
  ...
  if (view === "setup" && !activeTournament) return (
    <div style={{ minHeight: "100vh", background: DARK, color: "#fff", padding: 20, maxWidth: 500, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        {!isAdmin && (
          <div style={{ color: "#e63946", fontFamily: "monospace", fontSize: 11,
            textAlign: "center", marginBottom: 8 }}>
            🔐 Tap the role badge to unlock Scorer or Manager access
          </div>
        )}
        <button onClick={createTournament} disabled={selectedPlayers.length < 2 || !isAdmin}
          style={{ ...btnStyle(GOLD, DARK), opacity: (selectedPlayers.length < 2 || !isAdmin) ? 0.5 : 1 }}>
          🪓 Start Tournament
        </button>
      </div>
    </div>
                ...
                {isBye ? null : m.completed
                  ...
                  : isAdmin ? (() => {
                      ...
                      const playMatch = () => {
                        ...
                        if (isGrandFinal) {
                          onStartMatch({ id: m.id, p0: m.p0, p1: m.p1, tournamentId: activeTournament.id, matchType: "elimination", leagueId: activeTournament.isPlayoff ? "playoff" : undefined }, grandFinalSettings);
                        } else if (isDouble) {
                          onStartMatch({ id: m.id, p0: m.p0, p1: m.p1, tournamentId: activeTournament.id, matchType: "elimination", leagueId: activeTournament.isPlayoff ? "playoff" : undefined }, activeTournament.settings);
                        } else {
        ...
        {finalBannerMatch && (
          <div style={{ position: "fixed", inset: 0, zIndex: 300, background: "rgba(0,0,0,0.95)",
            ...
            <div style={{ background: "#0d0d0d", border: `2px solid #9b5de5`, borderRadius: 16,
              ...
              <button onClick={() => {
                onStartMatch({ id: finalBannerMatch.id, p0: finalBannerMatch.p0, p1: finalBannerMatch.p1, leagueId: activeTournament.isPlayoff ? "playoff" : undefined,
...
function LeagueScreen({ players, setPlayers, isAdmin, isManager, role, activeLeague, setActiveLeague, leagueHistory, setLeagueHistory, dayView, setDayView, onStartMatch, onStartPlayoff, onBack, notify }) {
  const isTablet = useTablet();
  ...
  const createLeague = (memberIds) => {
    if (!leagueName.trim()) { notify("Please enter a league name!"); return; }
    if (!memberIds || memberIds.length < 2) { notify("Need at least 2 members!"); return; }
    if (!isAdmin) { notify("Scorer access required"); return; }
    const schedule = buildLeagueSchedule(memberIds);
    const LEAGUE_SETTINGS = { throwsPerPlayer: 5, roundsPerMatch: 3, matchesPerPlayer: 3, days: 7 };
    ...
    // Build memberNames map so names survive player list changes
    const memberNames = {};
    leagueMembers.forEach(m => { memberNames[m.id] = m.name; });
  ...
  const getStandings = (leagueData) => {
    ...
    return leagueData.players
      .map(id => {
        ...
        // Name resolution: memberNames map (old format) or id-is-name (new format)
        const name = leagueData.memberNames?.[id] || (!/^\d{13}|^test_/.test(id) ? id : getPlayerName(id));
        return { id, name, ...t, winPct, ptsPer };
      })
  ...
  // League member management — separate from event players
  const [leagueMemberName, setLeagueMemberName] = useState("");
  const [leagueMembers, setLeagueMembers] = useState([]); // local list for new league setup

  const addLeagueMember = () => {
    if (!leagueMemberName.trim()) return;
    if (leagueMembers.find(m => m.name.toLowerCase() === leagueMemberName.trim().toLowerCase())) {
      notify("Member already in list"); return;
    }
    setLeagueMembers(prev => [...prev, { id: leagueMemberName.trim(), name: leagueMemberName.trim() }]);
    setLeagueMemberName("");
  };

  // Setup screen — no active league yet
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div style={cardStyle}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <h3 style={{ color: GOLD, margin: 0 }}>🏅 League Members ({leagueMembers.length})</h3>
          </div>
          {isAdmin && (
            <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
              <input value={leagueMemberName}
                onChange={e => setLeagueMemberName(e.target.value)}
                onKeyDown={e => e.key === "Enter" && addLeagueMember()}
                placeholder="Member name..."
                style={{ ...inputStyle, flex: 1 }} />
              <button onClick={addLeagueMember}
                style={{ ...btnStyle(GOLD, DARK), width: "auto", padding: "8px 16px", fontSize: 18 }}>+</button>
            </div>
          )}
          {/* Previous league members as quick-add chips */}
          {isAdmin && (() => {
            const allPastNames = new Set();
            [...(leagueHistory || [])].forEach(lg => {
              // New format: players are name-IDs directly
              (lg.players || []).forEach(id => {
                if (!/^\d{13}/.test(id) && !/^test_/.test(id) && !/^lm_/.test(id)) {
                  allPastNames.add(id);
                }
              });
              // Old format: memberNames map
              Object.values(lg.memberNames || {}).forEach(n => allPastNames.add(n));
            });
            const suggestions = [...allPastNames].filter(n =>
              !leagueMembers.find(m => m.name.toLowerCase() === n.toLowerCase())
            );
            if (suggestions.length === 0) return null;
            return (
              <div style={{ marginBottom: 10 }}>
                <div style={{ color: "#555", fontFamily: "monospace", fontSize: 11, marginBottom: 6 }}>
                  Previous members — tap to add:
                </div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {suggestions.map(name => (
                    <button key={name} onClick={() => {
                      if (leagueMembers.find(m => m.name.toLowerCase() === name.toLowerCase())) return;
                      setLeagueMembers(prev => [...prev, { id: name, name }]);
                    }} style={{ background: "#1a1a2a", border: "1px solid #1d6a96",
                      borderRadius: 20, padding: "4px 12px", color: "#4a90d9",
          ...
          {leagueMembers.length === 0 && (
            <div style={{ color: "#555", fontFamily: "monospace", fontSize: 12,
              textAlign: "center", padding: "8px 0" }}>
              Add at least 2 members to generate the schedule
            </div>
          )}
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {leagueMembers.map((m, i) => (
              <div key={m.id} style={{ display: "flex", alignItems: "center", gap: 5,
                background: "#1a1a1a", border: `1px solid ${GOLD}44`, borderRadius: 20,
                padding: "5px 12px" }}>
                <span style={{ color: "#555", fontFamily: "monospace", fontSize: 10 }}>#{i+1}</span>
                <span style={{ color: "#ccc", fontFamily: "monospace", fontSize: 13 }}>{m.name}</span>
                {isAdmin && (
                  <span onClick={() => setLeagueMembers(prev => prev.filter(x => x.id !== m.id))}
                    style={{ color: "#555", cursor: "pointer", fontSize: 15, marginLeft: 3 }}>×</span>
                )}
              </div>
            ))}
          </div>
        ...
        <button onClick={() => {
          if (!leagueName.trim()) { notify("Please enter a league name!"); return; }
          if (leagueMembers.length < 2) { notify("Need at least 2 members!"); return; }
          const dupName = (leagueHistory || []).find(l => l.name.trim().toLowerCase() === leagueName.trim().toLowerCase());
          if (dupName) { notify(`❌ "${dupName.name}" already exists — choose a different name`); return; }
          // Add new members to the shared player pool too
          const newPlayers = leagueMembers.filter(m =>
            !players.find(p => p.name.toLowerCase() === m.name.toLowerCase())
          );
          if (newPlayers.length > 0) setPlayers(prev => [...prev, ...newPlayers]);
          createLeague(leagueMembers.map(m => m.id));
        }} disabled={leagueMembers.length < 2 || !leagueName.trim() || !isAdmin}
          style={{ ...btnStyle(GOLD, DARK), opacity: (leagueMembers.length < 2 || !leagueName.trim() || !isAdmin) ? 0.5 : 1 }}>
          📅 Generate Schedule
        </button>

        {/* Master-only: instant test league */}
        {isMaster(role) && (
          <button onClick={() => {
            const testPlayers = ["Alex","Jordan","Sam","Taylor","Morgan","Casey"]
              .map((name, i) => ({ id: `test_${i}`, name }));
            // Add test players to room
            setPlayers(prev => {
              const existing = prev.map(p => p.name.toLowerCase());
              const newOnes = testPlayers.filter(p => !existing.includes(p.name.toLowerCase()));
              return [...prev, ...newOnes];
            });
            const ids = testPlayers.map(p => p.id);
            const schedule = buildLeagueSchedule(ids);
            const LEAGUE_SETTINGS = { throwsPerPlayer: 5, roundsPerMatch: 3, matchesPerPlayer: 3, days: 7 };
            const allMatches = schedule.flatMap((dayMatches, di) =>
              dayMatches.map(([p0, p1], mi) => ({
                id: `league_d${di}_m${mi}_test`, p0, p1, day: di, completed: false,
              }))
            );
            // Auto-complete everything except the last match of the last day
            const lastDay = Math.max(...allMatches.map(m => m.day));
            const lastDayMatches = allMatches.filter(m => m.day === lastDay);
            const lastMatchId = lastDayMatches[lastDayMatches.length - 1]?.id;
            const completedMatches = allMatches.map(m => {
              if (m.id === lastMatchId) return m; // leave last match incomplete
              // Randomly assign winner with fake scores
              const p0wins = Math.random() > 0.5;
              return {
                ...m, completed: true,
                winner: p0wins ? m.p0 : m.p1,
                p0rounds: p0wins ? 2 : 1, p1rounds: p0wins ? 1 : 2,
                scores: [[p0wins?15:8, p0wins?8:15],[p0wins?12:10, p0wins?10:12],[p0wins?14:9, p0wins?9:14]],
              };
            });
            setActiveLeague({
              id: `test_${Date.now()}`,
            ...
            setDayView(lastDay);
            notify(`🧪 Test league ready! ${lastDayMatches.length} match(es) left on Day ${lastDay + 1}`);
          }} style={{ ...btnStyle("#1a1a2a"), border: "1px solid #e63946", color: "#e63946", marginTop: 4 }}>
            🧪 Master: Fast-Forward Test League (6 players)
          </button>
        )}
      </div>
    </div>
