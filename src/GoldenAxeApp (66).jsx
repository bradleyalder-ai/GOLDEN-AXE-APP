          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {players.filter(p => playerTab === "all" || (playerTab === "members" ? p.member : !p.member)).map(p => (
              <div key={p.id} style={{
                ...
                {p.name}
                {isScorer && <span onClick={() => setPlayers(prev => prev.filter(x => x.id !== p.id))} style={{ color: RED, cursor: "pointer", fontSize: 16, lineHeight: 1, marginLeft: 8 }}>×</span>}
              </div>
            ))}
...
function progressDoubleElim(tournament, matchId, winnerId, loserId) {
  ...
  if (bracket === "w") {
    ...
    // Special case: if WB is now done AND there's only one LB player → bye straight to grand final
    const wbDone = bracketArr[ri].every(m => m.completed);
    const wbWinnersNow = wbDone ? bracketArr[ri].map(m => m.winner).filter(Boolean) : [];
    ...
    } else if (unpaired.length === 1 && wbWinnersNow.length === 1) {
      // Only 1 LB player and WB is done — bye through LB, straight to grand final
      const a = unpaired[0];
      lbRound[lbRound.indexOf(a)] = { ...a, completed: true, winner: a.p0, byeMatch: true };
      t.wbFinalist = wbWinnersNow[0];
      t.grandFinal = {
        id: "grand_final", p0: wbWinnersNow[0], p1: a.p0,
        roundNum: 99, slotIndex: 0, completed: false, byeMatch: false,
      };
      return t;
    } else if (unpaired.length === 1) {
      // Odd one out — bye (will play next WB dropin)
      const a = unpaired[0];
      lbRound[lbRound.indexOf(a)] = { ...a, completed: true, winner: a.p0, byeMatch: true };
    }
    ...
    if (bracketArr[ri].every(m => m.completed)) {
      ...
      // Check wbFinalist — may have been set above or earlier
      const wbF = t.wbFinalist;
      if (lbWinners.length === 1 && wbF) {
        // LB final done — set up grand final
        t.grandFinal = {
          id: "grand_final", p0: wbF, p1: lbWinners[0],
          roundNum: 99, slotIndex: 0, completed: false, byeMatch: false,
        };
