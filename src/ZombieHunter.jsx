    ...
    <svg viewBox="0 0 300 320" style={{ width: "100%", maxWidth: 320, display: "block", margin: "0 auto" }}>
      ...
      {[[-90], [90]].map(([dx], i) => (
        <g key={i} transform={`translate(${150+dx}, 50)`} style={s(!noNipple)} onClick={() => !noNipple && onScore({ id: "nipple", points: 7 })}>
          ...
          <text pointerEvents="none" x="0" y="5" textAnchor="middle" fill={noNipple ? "#555" : "#fff"} fontSize="11" fontFamily="monospace" fontWeight="bold">{noNipple ? "🚫" : "7"}</text>
        </g>
      ))}
...
export default function ZombieHunter({ onBack, roomPlayers = [] }) {
  ...
  const [selectedZombieIdxs, setSelectedZombieIdxs] = useState([0]); // indices of all starting zombies, first = champion
  ...
  const startGame = () => {
    ...
    let initialZombies, humans;
    if (startingZombies === 1) {
      // Single zombie — use championIdx
      const champId = ps[Math.min(championIdx, ps.length - 1)].id;
      initialZombies = [champId];
      humans = ps.filter(p => p.id !== champId).map(p => p.id);
    } else {
      // Multiple zombies — use selectedZombieIdxs (first = champion)
      const validIdxs = selectedZombieIdxs.filter(i => i < ps.length);
      const zombieIds = validIdxs.map(i => ps[i].id);
      initialZombies = zombieIds.length > 0 ? zombieIds : [ps[0].id];
      humans = ps.filter(p => !initialZombies.includes(p.id)).map(p => p.id);
    }
    setGame({
      allPlayers: ps,
      ...
      message: `🧟 ${ps.find(p => p.id === initialZombies[0])?.name} is the first zombie!`,
    });
    setGameOver(null);
    setStep("game");
  };
    ...
    <div style={{ minHeight: "100vh", background: DARK, color: "#fff", padding: 20, maxWidth: 500, margin: "0 auto" }}>
      ...
      {validNames.length >= 2 && (
        <div style={card}>
          ...
          {/* How many starting zombies */}
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
            {Array.from({ length: Math.max(1, validNames.length - 1) }, (_, i) => i + 1).map(n => (
              <button key={n} onClick={() => {
                setStartingZombies(n);
                setSelectedZombieIdxs([championIdx]);
              }} style={{
                flex: 1, minWidth: 44,
          ...
          {/* Single zombie — pick champion */}
          {startingZombies === 1 && (<>
            <div style={{ color: "#e63946", fontFamily: "monospace", fontSize: 11, marginBottom: 8 }}>
              WHO IS THE CHAMPION ZOMBIE?
            </div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {names.map((n, i) => n.trim() ? (
                <button key={i} onClick={() => { setChampionIdx(i); setSelectedZombieIdxs([i]); }} style={{
          ...
          {/* Multiple zombies — tap to select all */}
          {startingZombies > 1 && (()=> {
            const needed = Math.min(startingZombies, validNames.length - 1);
            return (<>
              <div style={{ color: "#e63946", fontFamily: "monospace", fontSize: 11, marginBottom: 8 }}>
                TAP TO SELECT {needed} STARTING ZOMBIES — first tapped = 👑 champion
              </div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {names.map((n, i) => {
                  if (!n.trim()) return null;
                  const selIdx = selectedZombieIdxs.indexOf(i);
                  const isSelected = selIdx >= 0;
                  const isChamp = selIdx === 0;
                  return (
                    <button key={i} onClick={() => {
                      setSelectedZombieIdxs(prev => {
                        if (prev.includes(i)) return prev.filter(x => x !== i);
                        if (prev.length < needed) return [...prev, i];
                        return prev;
                      });
                    }} style={{
                      ...btn(isChamp ? "#5a0000" : isSelected ? "#3a0a0a" : "#1a1a1a",
                        isChamp ? "#ff6666" : isSelected ? "#e63946" : "#aaa"),
                      border: `2px solid ${isChamp ? "#ff4444" : isSelected ? "#e63946" : "#333"}`,
                    }}>
                      {isChamp ? "👑🧟 " : isSelected ? "🧟 " : ""}{n.trim()}
                      {isSelected && <span style={{ color: "#888", fontSize: 10, marginLeft: 4 }}>#{selIdx+1}</span>}
                    </button>
                  );
                })}
              </div>
              <div style={{ color: "#666", fontFamily: "monospace", fontSize: 11, marginTop: 8 }}>
                {selectedZombieIdxs.length}/{needed} selected
                {selectedZombieIdxs.length > 0 && ` · Champion: ${names[selectedZombieIdxs[0]]?.trim()}`}
              </div>
            </>);
          })()}
        </div>
      ...
      <button onClick={startGame}
        disabled={validNames.length < 2 || (startingZombies > 1 && selectedZombieIdxs.length < Math.min(startingZombies, validNames.length - 1))}
        style={{ ...btn("#e63946"), width: "100%", padding: "14px", fontSize: 16,
          opacity: (validNames.length < 2 || (startingZombies > 1 && selectedZombieIdxs.length < Math.min(startingZombies, validNames.length - 1))) ? 0.5 : 1 }}>
