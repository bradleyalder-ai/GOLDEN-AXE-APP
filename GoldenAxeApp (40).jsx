    ...
    <div style={{ minHeight: "100vh", background: DARK, color: "#fff", padding: 20, maxWidth: 500, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 16 }}>
        ...
        {isAdmin && <button onClick={() => setShowSeedingConfirm(true)}
          style={{ ...btnStyle("#222"), border: "1px solid #555", width: "auto", padding: "6px 12px", fontSize: 12, marginLeft: "auto" }}>
                                  ...
                                  return (
                                    <button onClick={() => {
                                      if (isFinal && !activeTournament.isPlayoff) {
                                        setFinalSettings({ throwsPerPlayer: 5, roundsPerMatch: activeTournament.settings.roundsPerMatch || 1 });
                ...
                <div style={{ display: "flex", gap: 12, justifyContent: "center", marginBottom: 20 }}>
                  <div style={{ textAlign: "left" }}>
                    ...
                    <select value={finalSettings.throwsPerPlayer}
                      onChange={e => setFinalSettings(s => ({ ...s, throwsPerPlayer: Number(e.target.value) }))}
                      style={{ background: "#222", color: "#fff", border: "1px solid #bb86fc", borderRadius: 6, padding: "6px 10px", fontSize: 14, fontFamily: "monospace" }}>
                      {[3,4,5,6,7,8,9,10].map(n => <option key={n}>{n}</option>)}
                    </select>
                  </div>
                  <div style={{ textAlign: "left" }}>
                    ...
                    <select value={finalSettings.roundsPerMatch}
                      onChange={e => setFinalSettings(s => ({ ...s, roundsPerMatch: Number(e.target.value) }))}
                      style={{ background: "#222", color: "#fff", border: "1px solid #bb86fc", borderRadius: 6, padding: "6px 10px", fontSize: 14, fontFamily: "monospace" }}>
                      {[1,2,3,4,5].map(n => <option key={n}>{n}</option>)}
                    </select>
                  </div>
                ...
                <div style={{ display: "flex", gap: 12 }}>
                  <button
                    onClick={() => {
                      ...
                      onStartMatch({ ...finalBannerMatch, tournamentId: activeTournament.id, matchType: "elimination" }, { ...activeTournament.settings, ...finalSettings });
                    }}
